...
@section('title', $distributor->exists ? 'Editar distribuidor' : 'Nuevo distribuidor')
@section('subtitle', $distributor->exists ? 'Actualiza los datos del distribuidor' : 'Registra un nuevo distribuidor')

@section('content')

<div class="max-w-2xl">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
        <form action="{{ $distributor->exists ? route('inventario.distribuidores.update', $distributor) : route('inventario.distribuidores.store') }}"
            method="POST">
            @csrf
            @if($distributor->exists) @method('PUT') @endif
            ...
            <div class="mb-5">
                ...
                <input type="text" name="contact_name" id="contact_name" value="{{ old('contact_name', $distributor->contact_name) }}"
                    class="w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-2.5 text-sm focus:border-amber-400 focus:ring-amber-400/20 focus:ring-4 focus:bg-white transition-all">
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                ...
                <div>
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $distributor->email) }}"
                        class="w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-2.5 text-sm focus:border-amber-400 focus:ring-amber-400/20 focus:ring-4 focus:bg-white transition-all">
                </div>
            </div>
            ...
            <div class="mb-5">
                ...
                <textarea name="address" id="address" rows="2"
                    class="w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-2.5 text-sm focus:border-amber-400 focus:ring-amber-400/20 focus:ring-4 focus:bg-white transition-all">{{ old('address', $distributor->address) }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea name="notes" id="notes" rows="2"
                    class="w-full rounded-xl border-gray-200 bg-gray-50 px-4 py-2.5 text-sm focus:border-amber-400 focus:ring-amber-400/20 focus:ring-4 focus:bg-white transition-all">{{ old('notes', $distributor->notes) }}</textarea>
            </div>

            <div class="flex items-center justify-end gap-3 pt-2">
                <a href="{{ route('inventario.distribuidores.index') }}" class="px-5 py-2.5 text-sm text-gray-600 hover:text-gray-800 transition-colors">Cancelar</a>
                <button type="submit" class="px-6 py-2.5 bg-gray-900 text-white text-sm font-medium rounded-xl hover:bg-gray-800 transition-all shadow-sm">
                    {{ $distributor->exists ? 'Actualizar distribuidor' : 'Guardar distribuidor' }}
                </button>
            </div>
        </form>
    </div>
